...
def main(arguments):
  ...
  tf.logging.info('Select and save {} random pairs...'.format(args.num_lines))
  ...
  with open(args.outfile, 'w', encoding='iso-8859-1') as outputfile:
    ...
    random_idxs = np.random.choice(len(collected_pairs), args.num_lines,
      replace=False)
    for random_id in random_idxs:
      pair = collected_pairs[random_id]
      writer.writerow(pair)

  tf.logging.info('Wrote {} pairs to {}.'.format(args.num_lines, args.outfile))
